...
router.post('/auth', jsonParser, asyncMiddleware(async (req, res, next) => {
  debug('POST')
...
router.get('/auth', basic, asyncMiddleware(async (req, res) => {
  debug('GET /auth')
  let {user} = req
  let token = await user.createAuthToken()
  let signature = await hmac.hashData({token}, token)
...
router.put('/auth', bearer, jsonParser, asyncMiddleware(async (req, res) => {
  debug('PUT /auth')
...
router.get('/auth/verify/:id', asyncMiddleware(async (req, res) => {
